...
<body>
    ...
    if(isset($_GET["buscarpor"])){
        if($cat==0){
            $obras=obraspalabras($bd, $desplazamiento, $orden, $_GET["buscarpor"]);
            $total=totalobraspalabras($bd,$desplazamiento,$orden, $_GET["buscarpor"]);
        }
        else{
            $obras=obraspalabrasconcat($bd, $desplazamiento, $orden, $_GET["buscarpor"], $cat);
            $total=totalobraspalabrasconcat($bd, $desplazamiento, $orden, $_GET["buscarpor"], $cat);
        }
    }
    else{
    if($cat==0){
        $obras=obras($bd,$desplazamiento,$orden);
        $total=totalobras($bd);
    }else{
        $obras=filtrarobras1($bd, $desplazamiento, $orden, $cat);
        $total=totalobras1($bd, $desplazamiento, $orden, $cat);
...
?>
    ...
    <div class="row"> 
        ...
        <a class="noDecoration" <?php echo "href=obra.php?obra={$obras[$i]->getid()}";  ?> >
            ...
            <div class="card col-md-3 col-xs-12 col-sm-4 col-md-3 col-lg-3 mb-4">
